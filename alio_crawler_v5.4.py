...
# ── ALIO 공시항목 자동 수집 (v5.4) ────────────────
# 알리오 사이트의 공식 메뉴 API(formList.json)를 호출하여
# 전체 공시항목(83개, 4개 대분류)을 한 번에 수집한다.
...
def fetch_alio_items(progress_callback=None):
    ...
    응답 항목 구조:
        {
            "lcd": "2", "lcdnm": "기관운영",
            "nmcd": "RPTMGR0201", "nmcdnm": "일반현황",
            "mcd": "10105", "mcdnm": "일반현황",
            "scd": null, "scdnm": null,
            "reportYn": "Y", "reportType": "4",
            "reportNos": "10105", "quart": "0",
            "disclosureType": 4, ...
        }
...
def get_alio_items_cache_path():
    """캐시 파일 경로(crawler 스크립트와 같은 폴더)"""
    return os.path.join(os.path.dirname(os.path.abspath(__file__)), "alio_items.json")


def load_alio_items_cache():
    """캐시 로드 — 없거나 손상되면 None"""
    cache_path = get_alio_items_cache_path()
...
def save_alio_items_cache(items):
    """항목 메뉴를 캐시 파일에 저장"""
    cache_path = get_alio_items_cache_path()
    cache_data = {
...
def get_alio_items(force_refresh=False, progress_callback=None):
    """
    캐시 우선, 없거나 force_refresh=True면 API 재조회.
    반환: 항목 리스트
    """
    if not force_refresh:
        cached = load_alio_items_cache()
        if cached and cached.get("items"):
            return cached["items"]

    items = fetch_alio_items(progress_callback=progress_callback)
    if items:
        save_alio_items_cache(items)
    return items


def build_item_display_name(item):
    """
    항목 dict → 사용자에게 보여줄 항목명 생성.
    scdnm(세분류)이 있으면 그쪽이 더 구체적이므로 우선 사용.
    """
...
def build_item_root_no(item):
    """
    항목 dict → API 호출에 쓸 rootNo.
    reportNos에 콤마로 여러 개 있을 수 있다(예: "21211,21212,21213,21214").
    """
...
# ── 첨부파일 다운로드 일반화 (v5.4) ─────────────
# v5.3에서는 항목 type별로 다운로드 로직이 분화되어 있었으나
# v5.4부터는 4개 엔드포인트를 통합 함수로 호출한다.
...
def detect_endpoint_kind(item_meta):
    ...
    - 내부규정(mcd=21110) → "rule"
    - 사망자수+안전경영책임보고서(reportNos에 70401 포함) → "pdf+file+dfile"
...
def download_file_to_path(session, url, save_path, params=None, timeout=60):
    """
    파일 단일 다운로드. 같은 이름 충돌 시 (1), (2) 자동 부여.
    JSON 응답이 오면 API 에러로 간주.
    반환: (success: bool, saved_path: str, message: str)
    """
    try:
...
def fetch_board_attachment_list(session, apba_id, violation_meta):
    """
    v5.4.1: 게시판형 자료의 첨부파일 메타를 itemBoard{reportFormNo}.do HTML에서 추출.

    알리오 게시판형 첨부파일은 항목별로 두 가지 패턴이 혼재한다:
    - 패턴 A: ``downAttachFile('spath', 'sfile', 'dfile')``
              → /upload{spath}{sfile} 로 직접 GET
              (예: B1220 감사원 지적사항, B1210 국회지적사항)
    - 패턴 B: ``<a href="/download/download.json?fileNo=N">파일명</a>``
              → /download/download.json?fileNo=N 로 GET
...
def fetch_board_external_links(session, apba_id, violation_meta):
    """
    v5.4.1: 게시판형 자료의 외부 링크 추출 (입찰공고 B1030의 나라장터 g2b.go.kr 등).

    알리오 일부 게시판형은 첨부파일 대신 외부 사이트 링크만 제공.
    이 함수는 itemBoard{reportFormNo}.do HTML에서 외부 http(s) 링크를 추출한다.
...
def download_board_attachment(session, attachment, save_dir):
    """
    v5.4.1: 게시판형 첨부파일 1건 다운로드 (두 패턴 통합).

    kind="upload": ``/upload{spath}{sfile}`` 직접 GET (감사원 지적사항 등)
    kind="fileno": ``/download/download.json?fileNo=N`` GET (임원 모집공고 등)
    """
...
def download_attachment(session, kind, file_info, save_dir,
    ...
    file_info: {"id": "...", "name": "...."}
    save_dir : build_save_path()로 만든 폴더
...
def load_public_institutions(progress_callback=None):
    """ALIO 기관목록 API에서 기관 목록 로드 (지역 정보 포함) - 병렬 처리"""
    try:
